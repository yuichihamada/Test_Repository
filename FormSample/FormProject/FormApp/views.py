...
def form_page(request):
  ...
  if request.method == 'POST':
    ...
    if form.is_valid(): #バリデーション（フィールドが正しいかチェック)
      print('バリデーション成功')
      # print('{}, mail:{},age:{}'.format(form.cleaned_data['name'], form.cleaned_data['mail'], form.cleaned_data['age']))
...
def modelform_set_post(request):
  ...
  TestFormSet = modelformset_factory(ModelSetPost, form=forms.ModelFormSetPost, fields='__all__', extra=3)
